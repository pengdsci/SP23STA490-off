---
title: "Week 6: Ramdom Sampling Plans "
author: "Cheng Peng"
date: "  "
output:
  html_document: 
...
Based on the above frequency distribution of the modified 2-digit NAICS codes (the 3-digit codes are combined categories). We use the following inclusion rule to define the study population: excluding small-size categories
20, 21, 55, 92, and unclassified business with NAICS code 0.
...
3. Stratified sample. 
   ...
   In order to find a stratified sample, you need to choose a variable to define a stratification variable and then use this stratification variable to take a stratified sample


You need to write a report that is similar to this note. To be more specific, you are expected to include the following components in your write-up"

(1). The description of simple random sampling and code that takes a simple random sample with a pre-determined sample size. 

(2). The description of the Systematic sampling and the code that takes a systematic sample with a predetermined sample size.

(3). The description of stratified sampling and the code that takes a stratified sample with a predetermined sample size.

    In the stratified sampling, you need to describe in detail how the stratification variable was defined (modified) and the rationale for selecting that variable for the stratification.
