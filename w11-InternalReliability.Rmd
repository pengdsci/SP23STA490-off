...
In other words, in order for the survey results to be useful, the survey must demonstrate both reliability and validity.
...
```{r echo=FALSE, fig.align='center', fig.cap="Figure 1: Validiy vs reliability of a survey instrument", out.width = '40%'}
include_graphics("w11-validity-reliability.jpg")
```
...
Operationalizations are a researcher’s translation of theoretical variables into procedures that make them observable.


## Types of validity

In survey design, a <font color = "red">**construct**</font> is the abstract idea, underlying theme, or subject matter that one wishes to measure using survey questions. 

There are many different versions of descriptions of different types of validity. The following are modern descriptions of the validity of survey construct.
...
* **Generalizability** examines the extent to which scores generalize across different population groups, different situations or settings, different time periods, and/or to other operationalizations representative of the construct domain.

* **External Validity** refers to the comparison of the operationalization of interest to external measures (see the earlier discussions of convergent, divergent, and predictive validity). This is assessed by investigating the relationship between the operationalization and other variables

* **Consequential Validity** includes gathering evidence and rationales for evaluating the consequences of score interpretations from a survey. Researchers should accrue evidence of positive consequences and evidence that adverse consequences are minimal.


## Improving Validity

The strategy of improving the validity of survey instruments is to reduce (or even avoid) bias. The following are common potential biases in questionnaire design. 
...
* **Complex bias** - is the tendency to look at something that is easy to understand, or look at it when we are in a state of confusion, and view it as having many parts that are difficult to understand. Questions should be worded as simply and clearly as possible. If technical jargon must be used, it is best to define the term specifically in the survey. Finally, we should **avoid** using double-barreled questions in any survey.


# Reliability

In order for survey results to be useful, the survey must demonstrate reliability. The best practices for questionnaire design is to help maximize the instrument’s reliability.

Based the definition given earlier, the reliability is a quality measure of the survey instrument. We can use quantitative approaches to analyze the reliability of the survey. There are several different types of reliability measures available in literature and practice. We will introduce some commonly used ones.
...
Having parallel forms is useful for studies involving both a pre-test and post-test in which the researcher does not want to use the same form at both time periods (think of why they may want to avoid this). If parallel forms reliability can be established, then the researcher can use both forms in their study.
...
There are different internal consistency measures that can be used. The following are commonly used in practice.
...
For example,  we have six items in the gratitude survey. There have 15 different item pairings (i.e., 15 correlations). We take average all 15 correlation coefficients as illustrated in the following figure.

```{r echo=FALSE, fig.align='center', fig.cap="Figure 2: Inter-item correlation", out.width = '40%'}
include_graphics("w11-Inter-correlation-gratitude.jpg")
```

**Caution**: To get truthful information from respondents, researcher intentionally reverse the scale. In this case, we need to calculate the mean absolute correlation. 
...
include_graphics("w11-Cronbach-Alpha.jpg")
```
...
In the above data, variables 2-4 are at the same scale. So we define a dataframe that contains only the three items (variables) at the sample scale to calculate Cronbach's Alpha of the scale.

```{r}
# need to load package {psych} first.
example.data = read.csv("https://raw.githubusercontent.com/pengdsci/STA490/main/w11-CronbachAlpha-dataset.csv")
...
Cronbach's $\alpha$ is also called the coefficient of reliability that ranges from 0 to 1. It provide the overall assessment of a measure’s reliability of the underlying scale. 

* If all of the scale items are entirely independent from one another (i.e., are not correlated or share no covariance), then  $\alpha$  = 0; 
...
There three major ways of administering a survey. Which one is used is dependent on the scope of the study and factors such as
project budget, characteristics of the study population, the sampling frame, desired response rate, and the characteristics of the survey, etc.
